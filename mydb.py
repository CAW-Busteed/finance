...
def buy_main(db, rows, user_id, time, stock_quote, share_num, cost, buyorsell):
    ...
    db.execute(
        ...
        user_id, str(time), stock_quote.upper(), share_num, cost, buyorsell)
    asset = db.execute("SELECT * FROM assets WHERE user_id = ? AND stock = ?",
...
def buy_update(asset, db, user_id, stock_quote, share_num, stock_dic, cost):
    if len(asset) == 0:
        db.execute(
            ...
            user_id, stock_quote.upper(), share_num, stock_dic["price"], cost)
    else:
        ...
        db.execute(
            ...
            number, stock_dic["price"], total_value, user_id,
            stock_quote.upper())
    return True
...
def sell_variables(db, user_id, stock_quote, stock_dic, share_num):
    ...
    assets = db.execute("SELECT * FROM assets WHERE user_id = ? AND stock = ?",
                        user_id, stock_quote.upper())
    cost = stock_dic["price"] * float(share_num)
    buyorsell = "sell"
    now = datetime.now()
...
def sell_main(number, db, stock_dic, total_value, user_id, stock_quote):
    if number > 0:
        ...
        db.execute("DELETE FROM assets WHERE user_id = ? AND stock = ?",
                   user_id, stock_quote.upper())
    return True


def sell_update(db, user_id, gains, time, stock_quote, share_num, cost,
                buyorsell):
    db.execute("UPDATE users SET cash=? WHERE id = ?", gains, user_id)
    db.execute(
        "INSERT INTO transactions (user_id, date, company, shares, total_cost, type) VALUES (?, ?, ?, ?, ?, ?)",
        user_id, time, stock_quote.upper(), share_num, cost, buyorsell)
    return True
